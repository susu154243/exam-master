...
- 三大模式卡片置顶（练习模式 / 今日复习 / 考试模式）
- 每道题做题次数统计（答题次数/正确率/评分/下次复习）
- 筛选（全部/新题/掌握良好/需加强/高频）+ 排序
- 新增 `get_question_attempt_stats()` 模型函数

## v0.9.0 - 2026-04-30
**学习设置页全面升级**

- 练习设置 → 学习设置
- 今日复习板块（到期题目列表 + 一键开始复习）
- 学习进度条
- 已掌握进度条
- 评分分布进度条（忘了/模糊/一般/简单/秒答）
- 新增 `get_due_today()` 和 `get_study_progress()` 模型函数
- 新增 `/study/<cat>/today` 今日复习入口路由
